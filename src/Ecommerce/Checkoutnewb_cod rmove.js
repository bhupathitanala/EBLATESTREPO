...
function Checkoutnew() {
    ...
    //     if (user?.ID) {
    //         Promise.all([
    //             Apicalls.get('cart/items/' + user.ID),
        ...
        if(addressLength === 0 || selectedAddress === null){
            ...
            // const jwtToken = storage.token;
                ...
                try {
                    ...
                    const options = {
                        key: 'rzp_live_vEMJqi68CvzaXh', // Enter the Key ID generated from the Dashboard
    ...
    console.log(parseInt(calculateSubtotal()))
                ...
                <div id="collapseTwo" className="accordion-collapse collapse show" aria-labelledby="headingTwo" data-bs-parent="#checkoutAccordion" >
                    ...
                    <div className='d-flex flex-column'>
                                    ...
                                    <div className='text-end' >
                                        ...
                                        {/* <p className='total_price mb-0'>Total : ₹ {(parseFloat(cartItems.reduce((total, item) => total + (item.salePrice === 0 ? item.price : item.salePrice) * item.quantity, 0)) + parseFloat(totalShippingCharges) + (selectedOption === "0" ? parseFloat(totalCodCharges) : 0)).toFixed(2)} </p>finalTotal */}
